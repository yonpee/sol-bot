...
async function checkRaydiumRoute(tokenMint, lamports) {
  try {
    ...
    if (!quote || !quote.success) {
      console.log("Raydiumルートなし: " + (quote ? quote.msg : "不明"));
      return null;
    }
...
async function buyToken(tokenMint, solPriceUsd, isPumpFun) {
  const { getBotConfig } = require("./api");
  const config = getBotConfig();
  const buyAmountUsd = config.buyAmount || TRADE_CONFIG.BUY_AMOUNT_USD;

  console.log("SOL購入開始: " + tokenMint);
  console.log("購入金額: $" + buyAmountUsd);

  try {
    const wallet = getWallet();
    const connection = getConnection();
    const lamports = await usdToLamports(buyAmountUsd, solPriceUsd);
    console.log("買い金額: $" + buyAmountUsd + " = " + lamports + " lamports");
    ...
    if (!txRes.data || !txRes.data.success) {
      console.error("トランザクション失敗:", txRes.data ? txRes.data.msg : "不明");
      return null;
    }
  ...
  try {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        ...
        const quote = quoteRes.data;
        if (!quote || !quote.success) {
          console.error("売りクォート失敗(" + attempt + "): " + (quote ? quote.msg : "不明"));
        ...
        if (!txRes.data || !txRes.data.success) {
          console.error("売りトランザクション失敗(" + attempt + "): " + (txRes.data ? txRes.data.msg : "不明"));
          await new Promise(function(r) { setTimeout(r, 2000); });
          continue;
        }
        ...
        for (let i = 0; i < transactions.length; i++) {
          ...
          try {
            ...
            return { txid: txid, reason: reason, currentPrice: currentPrice };
          } catch (confirmError) {
...
module.exports = { buyToken: buyToken, sellToken: sellToken, TRADE_CONFIG: TRADE_CONFIG };
